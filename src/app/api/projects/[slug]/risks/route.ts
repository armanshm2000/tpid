...
import { demoProjects, demoRisks, demoUsers } from "@/lib/demo-data";
...
export async function GET(_request: Request, { params }: { params: { slug: string } }) {
  ...
  try {
    const risks = await prisma.risk.findMany({ where: { projectId: pid }, include: { author: { select: { id: true, name: true } } } });
    return NextResponse.json(risks);
  } catch {
    return NextResponse.json(demoRisks.filter((r) => r.projectId === pid).map((r) => ({ ...r, author: demoUsers.find((u) => u.id === r.authorId) })));
  }
}
